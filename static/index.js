...
(function(w) {
    ...
    function shareText(cps) {
        ...
        if (cps <= 3) return '冰冰：放假了';
        if (cps <= 4)  return '冰冰：我不接受你的理由';
        if (cps <= 5) return '冰冰：这题怎么能做错呢？';
        if (cps <= 6) return '冰冰：我想静静';
        if (cps <= 7) return '冰冰：很显然cps5-6的内容是病句';
        if (cps <= 8) return '冰冰：你已经掌握吃冰的真谛了';
        return '地球生活的可好？';
    }
